...
RUN apt-get install -y --force-yes build-essential curl openssl libssl-dev git-core vim tklib zlib1g-dev libssl-dev libreadline-gplv2-dev libxml2 libxml2-dev libxslt1-dev && \
      ...
      su buildbox /bin/bash --login -c "rbenv install 1.9.3-p545 && rbenv local 1.9.3-p545 && gem update --system && gem install bundler && rbenv rehash" && \
      su buildbox /bin/bash --login -c "rbenv install 2.0.0-p247 && rbenv local 2.0.0-p247 && gem update --system && gem install bundler && rbenv rehash" && \
      su buildbox /bin/bash --login -c "rbenv install 2.1.0 && rbenv local 2.1.0 && gem update --system && gem install bundler && rbenv rehash" && \
      su buildbox /bin/bash --login -c "rbenv install 2.1.1 && rbenv local 2.1.1 && gem update --system && gem install bundler && rbenv rehash" && \
      su buildbox /bin/bash --login -c "rbenv install 2.1.2 && rbenv local 2.1.2 && gem update --system && gem install bundler && rbenv rehash" && \
      su buildbox /bin/bash --login -c "rbenv global 2.1.2"
...
RUN apt-get install -y --force-yes curl && \
      ...
      VERSION="0.2-beta.10" DESTINATION="/home/buildbox/.buildbox" bash -c "`curl -sL https://raw.github.com/buildboxhq/buildbox-agent/master/install.sh`"
